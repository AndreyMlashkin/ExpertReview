    ...
    : QAbstractItemView(parent)
{
    ...
    m_margin = 8;
    m_totalSize = 300;
    m_pieSize = m_totalSize - 2 * m_margin;
    m_validItems = 0;
    m_totalValue = 0.0;
    m_rubberBand = 0;
}
...
QModelIndex PieView::indexAt(const QPoint &point) const
{
    ...
    if (wx < m_totalSize) {
        double cx = wx - m_totalSize / 2;
        double cy = m_totalSize / 2 - wy; // positive cy for items above the center
...
QRect PieView::itemRect(const QModelIndex &index) const
{
    ...
    case 0:
        ...
        return QRect(m_totalSize,
                     int(m_margin + listItem * itemHeight),
                     m_totalSize - m_margin, int(itemHeight));
    case 1:
    ...
    for (int row = 0; row < model()->rowCount(rootIndex()); ++row) {
        ...
        if (value > 0.0) {
            ...
            if (sliceIndex == index) {
                ...
                slicePath.moveTo(m_totalSize / 2, m_totalSize / 2);
                slicePath.arcTo(m_margin, m_margin, m_margin+m_pieSize, m_margin+m_pieSize,
                                startAngle, angle);
...
void PieView::paintEvent(QPaintEvent *event)
{
    ...
    for (row = 0; row < model()->rowCount(rootIndex()); ++row)
    {
        ...
        if (value > 0.0)
        {
            ...
            painter.drawPie(0, 0, m_pieSize, m_pieSize, int(startAngle*16), int(angle*16));
    ...
    for (int row = 0; row < rows; ++row)
    {
        for (int column = 0; column < columns; ++column)
        {
            ...
            if (region.intersects(contentsRect))
            {
                ...
                int correspondingColumn = (column == 0) ? 1 : 0;
                QModelIndex correspondingInd = model()->index(row, correspondingColumn, rootIndex());
                indexes.append(correspondingInd);
            }
        }
    }
...
void PieView::updateGeometries()
{
    ...
    horizontalScrollBar()->setRange(0, qMax(0, 2*m_totalSize - viewport()->width()));
    verticalScrollBar()->setPageStep(viewport()->height());
    verticalScrollBar()->setRange(0, qMax(0, m_totalSize - viewport()->height()));
}
